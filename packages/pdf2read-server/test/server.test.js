...
test('POST /reflow with a real PDF returns real reading blocks, matching the library directly', async () => {
  const { proc, baseUrl } = await startServer();
  try {
    const pdfBytes = await readFile(fixture('2608.11433.pdf'));
    const res = await fetch(`${baseUrl}/reflow`, {
      method: 'POST',
    ...
    assert.ok(allBlocks.some(b => b.type === 'heading' && /Stigma and Support/.test(b.text)));

    const { pdfToReadingBlocks } = await import('@pdfree/pdf2read-core');
    const direct = await pdfToReadingBlocks(pdfBytes);
    // Compare through the SAME JSON round-trip on both sides, not the raw
    // in-memory object against the parsed response — a block's undefined
    // `fontFace` is a real, present key in the raw object (e.g. a bullet
    // item with no resolved font) but JSON.stringify drops undefined-valued
    // keys entirely, so the raw and wire-format shapes are legitimately
    // different even though the content is identical. Found by this test
    // itself on the first real run — not a server bug, just an unfair
    // comparison between two different serialization states.
    assert.deepEqual(result, JSON.parse(JSON.stringify(direct)), 'server output must be identical to calling the library directly');
  } finally {
    await stopServer(proc);
  }
});

test('POST /reflow with wrong Content-Type returns 400', async () => {
  const { proc, baseUrl } = await startServer();
  try {
    const res = await fetch(`${baseUrl}/reflow`, {
      method: 'POST',
...
test('POST /reflow with an empty body returns 400', async () => {
  const { proc, baseUrl } = await startServer();
  try {
    const res = await fetch(`${baseUrl}/reflow`, {
      method: 'POST',
...
test('POST /reflow with a non-PDF body (wrong content, right Content-Type) is rejected by the %PDF- signature check, before ever spawning a worker', async () => {
  const { proc, baseUrl } = await startServer();
  try {
    const res = await fetch(`${baseUrl}/reflow`, {
      method: 'POST',
...
test('SECURITY: a slow conversion is really cut off by TIMEOUT_MS, not just client-facing (worker.terminate() enforcement, not a same-thread race)', async () => {
  ...
  try {
    // A real, multi-page PDF that takes real time to reflow — 200ms is
    // nowhere close to enough for a genuine conversion to finish, so a 504
...
test('SECURITY: with MAX_CONCURRENT=1, a second simultaneous request queues and still succeeds once the first finishes', async () => {
  ...
  try {
    const pdfBytes = await readFile(fixture('2608.11433.pdf'));
    const post = () => fetch(`${baseUrl}/reflow`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/pdf' },
      body: pdfBytes,
    });
    ...
    const [jsonA, jsonB] = await Promise.all([a.text(), b.text()]);
    assert.equal(jsonA, jsonB, 'both requests convert the same file — output must match');
  } finally {
    await stopServer(proc);
  }
});
