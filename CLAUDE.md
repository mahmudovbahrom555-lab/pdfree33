...
- `js/vendor/` — bundled third-party libraries. "Never modify" means never hand-edit the bundled
  code — it does NOT forbid a deliberate version bump (copy the library's own official build
  output over the vendored file), which is a normal maintenance operation, just one to do
  carefully (see the pdf-lib note right below) and not casually.
- `js/worker.js` — Web Worker (heavy PDF ops, off-limits)
- `sw.js` — Service Worker (auto-generated by build.py, do not edit directly)

**`js/vendor/pdf-lib.min.js` version-sync note**: this file is a manual copy of
`node_modules/pdf-lib/dist/pdf-lib.min.js` — `pdf-lib` is a `devDependency` (package.json), used
by the Node test suite via `node_modules`, but the actual browser/worker runtime loads the
separately-vendored copy, which `package-lock.json`/Dependabot has zero visibility into.
`scripts/check_vendor_sync.py` (wired into `.husky/pre-commit`, `npm run check:vendor`, and CI)
fails loudly if the two ever drift apart. If it fails, don't just re-sync and move on — this
project's `js/worker.js` carries version-specific workarounds for real pdf-lib bugs found by
direct source inspection (`_cleanDanglingAnnots()` — see memory
`flatten_dangling_annots_bug_2026_09` / `fill_market_research_radio_annots_bug_2026_09`), so any
pdf-lib bump needs `npm test` plus a live-browser smoke pass on Fill/Flatten/Split/Extract/Protect
before it's safe, not just a file copy.
